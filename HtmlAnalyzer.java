...
public class HtmlAnalyzer {
    ...
    private static List<String> fetchHtmlLines(String url) throws FetchException {
        try {
            ...
            if (!uri.isAbsolute()) {
                throw new FetchException("Non-absolute URL");
            }
            String scheme = uri.getScheme();
            if (scheme == null) {
                throw new FetchException("Missing URL scheme");
            }
            String s = scheme.toLowerCase();
            if (!s.equals("http") && !s.equals("https")) {
                throw new FetchException("Unsupported URL scheme: " + scheme);
            }
    ...
    private static Result analyze(List<String> lines) {
        ...
        for (String raw : lines) {
            ...
            if (line.startsWith("<")) {
                ...
                if (mOpen.matches()) {
                    stack.push(mOpen.group(1).toLowerCase());
                    continue;
                }

                Matcher mClose = CLOSE_TAG.matcher(line);
                if (mClose.matches()) {
                    String tag = mClose.group(1).toLowerCase();
